...
void WVertex::computeNormal() {
    ...
    do {
        ...
        if(tmp->end() == this && tmp->left()!= NULL) {
            average += tmp->left()->normal();
            tmp = tmp->succLeft();
            cpt++;
        } else if(tmp->begin() == this && tmp->right()!= NULL) {
            average += tmp->right()->normal();
            tmp = tmp->succRight();
            cpt++;
        } else if(tmp->end() == this && tmp->right()!= NULL) {
            average += tmp->right()->normal();
            tmp = tmp->succLeft();
            cpt++;
        } else if(tmp->begin() == this && tmp->left()!= NULL) {
            average += tmp->left()->normal();
            tmp = tmp->succRight();
            cpt++;
        } else {
    ...
    if(cpt==0) {
        cerr << "ERROR computeNormal: premier tour de boucle échoué." << endl;
        return;
    }

    // Divide par le nombre de points
    average /= cpt;
...
void WVertex::drawLineCatmull() {
    WEdge *e=this->edge();
    do {
        p3d::drawLines(std::vector<p3d::Vector3>{this->pointVertex(),e->pointEdge()});
        if (this==e->begin())
            e=e->predLeft();
        else
            e=e->predRight();
