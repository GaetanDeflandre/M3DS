...
void GLApplication::updateCamera() {
    ...
    case Camera_Follow_Car: {
        ...
        // Postion camera repère voiture
        Vector3 pos_cam_voiture = Vector3(0,2,8);

        // Position camera repère monde
        Vector3 pos_cam_world = _car.position() + _car.orientation()*pos_cam_voiture;

         /*
        _camera.orientation(_car.orientation());
        _camera.position(pos_cam_world);
        //*/

        // Question 17
        _cameraStop.orientation(_car.orientation());
        _cameraStop.position(pos_cam_world);
    ...
    case Camera_Follow_Plane: {
        ...
        // Postion camera repère avion
        Vector3 pos_cam_airplane = Vector3(0,1,-3);

        // Position camera repère monde
        Vector3 pos_cam_world = _airplane.position() + (_airplane.orientation()*pos_cam_airplane);
