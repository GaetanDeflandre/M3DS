...
TreeBSP *ObjectBSP::consBSP(const vector<FaceBSP *> &tabFace) {
    ...
    if (tabFace.empty()) {
        ...
        unsigned size = tabFace.size();
        FaceBSP* pivot = tabFace[0];

        if(pivot == NULL){
            cerr << "ERROR consBSP: le pivot est null." << endl;
            return NULL;
        }
        ...
        for(unsigned i=1; i<size; i++){
            ...
            FaceBSP* current = tabFace[i];

            if (current == NULL){
                cerr << "ERROR consBSP: face null dans la table de face." << endl;
                return NULL;
            }

            current->separe(*pivot);
            FaceBSP* fNeg = current->faceNegative();
            FaceBSP* fPos = current->facePositive();

            if(fNeg != NULL){
                listeNegative.push_back(fNeg);
            }
            if(fPos != NULL){
                listePositive.push_back(fPos);
            }
        }
...
void ObjectBSP::drawBSP(TreeBSP *tree,const Vector3 &eye) {
    ...
    // On est au fond de l'abre, fin récursion
    if(tree==NULL){
        return;
    }

    FaceBSP* node = tree->node();

    if(node == NULL){
        cerr << "ERROR drawBSP: arbre avec noeud null" << endl;
        return;
    }

    ESign eye_sign = node->sign(eye);
    ...
    case SIGN_NONE:
        cerr << "ERROR drawBSP: eye sign is none" << endl;
        break;

    default:
        cerr << "ERROR drawBSP: eye sign is invalid" << endl;
        break;
    }
}
