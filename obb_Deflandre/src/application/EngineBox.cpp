...
void EngineBox::euler(double dt) {
    for(unsigned int i=0; i<_boxList->size(); i++) {
        ...
        // à compléter
        Vector3 omegaOld = b->omega();
        double thetaOld = b->theta();

        b->omega(omegaOld + (b->moment()/b->inertia()) * dt);

        // omegaOld = vitesse angulaire : dt*b->omega().z()
        b->theta(thetaOld + dt*b->omega().z());
