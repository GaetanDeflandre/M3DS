...
void GLApplication::moveSelectedObject() {
    if (mouseRight()) {
        if (_selectedIntersection) { // an intersection has been found (in GLApplication::selectObject() ).
            ...
            double dx = double(deltaMouseX())/100.0;
            double dy = double(deltaMouseY())/100.0; // /100.0 to attenuate mouse motion

            if (_controlMouse==Manipulation_Translation) {

                Vector3 pw = _camera.windowToWorld(dx, dy);
                Vector3 po = mesh->pointTo(Coordinate_Local, pw);
                mesh->translate(po,Coordinate_Local);
            }
...
void GLApplication::updateCamera() {
    ...
    if (left()) {
        _camera.translate(Vector3(-speed,0,0),Coordinate_Local);
    }
    if (right()) {
        _camera.translate(Vector3(speed,0,0),Coordinate_Local);
    }
    if (forward()) {
        _camera.translate(Vector3(0,0,-speed),Coordinate_Local);
    }
    if (backward()) {
        _camera.translate(Vector3(0,0,speed),Coordinate_Local);
    }
    if (mouseLeft()) {
        ...
        // rotation de la tête autour de l'axe X
        _camera.rotate(deltaMouseY()/2,Vector3(1,0,0),Coordinate_Local);
        // rotation du corps autour de l'axe Y (solution au tangage)
        _camera.rotate(-deltaMouseX()/2,Vector3(0,1,0),Coordinate_World);
    }
